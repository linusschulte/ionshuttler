...
import reproduce.paper.run as paper_run


def test_quick_run_writes_outputs_extends_and_resumes(tmp_path, monkeypatch) -> None:
    output = tmp_path / "paper"
    assert paper_run.main(("quick", "--workers", "1", "--output", str(output))) == 0
    expected = (
        ...
        output / "checkpoints" / "table_ii_000000.csv",
    ...
    assert paper_run.main(("paper", "--plots-only", "--output", str(output))) == 0
    ...
    assert paper_run.main(("quick", "--samples", "2", "--workers", "1", "--output", str(output))) == 0
    with (output / "csv" / "raw_trajectories.csv").open(newline="", encoding="utf-8") as handle:
    ...
    assert (output / "checkpoints" / "table_ii_000001.csv").is_file()
    ...
    monkeypatch.setattr(paper_run, "simulate_state", fail_if_simulated)
    assert paper_run.main(("quick", "--samples", "2", "--workers", "1", "--output", str(output))) == 0
