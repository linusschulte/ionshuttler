...
def main(config: dict[str, Any]):
    ...
    if qasm_file_path_cfg is not None:
        ...
        qasm_file_path = (
            qasm_base_dir
            / (algorithm_name + f"{gate_densities_string}")
...
def predictor_test_function(config: dict[str, Any]) -> None:
    qasm_folder = config.get("qasm_folder")
    if not qasm_folder:
        msg = "Config parameter 'qasm_folder' is required but not set"
        raise ValueError(msg)

    qasm_dir = pathlib.Path(qasm_folder)
    if not qasm_dir.is_dir():
        msg = f"QASM folder does not exist or is not a directory: {qasm_dir}"
        raise ValueError(msg)
    ...
    if n_circuits is not None:
        qasm_files = qasm_files[:n_circuits]

    if not qasm_files:
        print(f"No QASM files found in {qasm_dir}")
        return

    results_file = config.get("results_file", "outputs/simulation_results.h5")
    ...
    with h5py.File(results_path, "a") as f:
        if "results" not in f:
            ...
            f.attrs["qasm_folder"] = str(qasm_dir)
        ...
        for qasm_file in qasm_files:
            ...
            if target_qubits is not None and num_ions != target_qubits:
            ...
            try:
                ...
                print(f"Skipping {qasm_file}: could not count gates ({exc})")
                skipped += 1
                continue

            run_config = config.copy()
            run_config["algorithm_name"] = circuit_name
            ...
            try:
                ...
                if final_timesteps >= run_config.get("max_timesteps", 100000) - 1:
                ...
                run_group.attrs["idle_count"] = final_timesteps * num_ions - gate_count
                if time_slices_info:
...
if __name__ == "__main__":
    ...
    if args.run_meta_study.lower() != "y":
        if config.get("qasm_folder"):
            predictor_test_function(config)
        else:
